...
function createCollectionService(tree: UnitTestTree, options: Schema) {
  return runner
    .runSchematicAsync('collection-service', options, tree)
    .toPromise();
}
...
describe('CollectionService', () => {
  ...
  it('Should create a CollectionService', async () => {
    ...
    expect(
      tree.files.includes('/projects/foo/src/app/movie.service.ts')
    ).toBeTruthy();
  });
  ...
  it('Should create a CollectionService in another project', async () => {
    ...
    const tree = await createCollectionService(appTree, options);
    expect(
      tree.files.includes('/projects/bar/src/app/movie.service.ts')
    ).toBeTruthy();
  });
  ...
  it('Should have NO test file by default', async () => {
    ...
    const tree = await createCollectionService(appTree, options);
    expect(
      tree.files.includes('/projects/foo/src/app/movie.service_spec.ts')
    ).toBeFalsy();
  });
  ...
  it('Should have test file with option', async () => {
    ...
    const tree = await createCollectionService(appTree, options);
    expect(
      tree.files.includes('/projects/foo/src/app/movie.service.spec.ts')
    ).toBeTruthy();
  });
});
